...
def train_AL(dataloaders, model, optimizer, scheduler, num_epochs=25,
             model_dir='', model_name_prefix='', device='cuda', balance=1.):
    """
        Training under OPD-AL strategy
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for input_lowdose in dataloaders[phase]:
                input_lowdose = input_lowdose.to(device)

                optimizer.zero_grad()

                with torch.set_grad_enabled(phase == 'train'):
                    output_stddose = input_lowdose
                    for i in range(input_lowdose.size(0)):
                        for j in range(input_lowdose.size(1)):
                            output_stddose[i][j] = model(input_lowdose[i][j])
                    loss = calc_loss_AL(output_stddose, input_lowdose, metrics, balance)

                    if phase == 'train':
                        loss.backward()
                        optimizer.step()

                epoch_samples += input_lowdose.size(0) * input_lowdose.size(1)
